...
def bobs_custom_pwhash_attack(pc, char_list, length, pwlist_obj):
    ...
    import operator
    pwhash_length = len(pwlist_obj[0].get('hash', ''))          # Acquire the length of the entire password hash
    l = pwhash_length//3                            # Derive a third of the password hash length, converting to an integer
    def create_truncated_hash_fn(suffix, length):
        def truncated_hash_fn(p):
            return crypto.truncated_sha2_hex_digest(p + suffix, length)
        return truncated_hash_fn

    pwhash_fn_p1_truncated = create_truncated_hash_fn('-p1', 6)  # Define hash function for p1
    pwhash_fn_p2_truncated = create_truncated_hash_fn('-p2', 6)  # Define hash function for p2
    hash_p1, hash_p2, salt_c, salts, pwhashs = ([] for _ in range(5)) 
    ...
    for i, pwitem in enumerate(pwlist_obj):
        ...
        ## TODO ##
        ## Insert your code here
        hash_p1.append(pwhash[:l])  # Extracting and storing the hash of p1 from the provided hash
        hash_p2.append(pwhash[l:2*l])  # Extracting and storing the hash of p2 from the provided hash
        salt_c.append(pwhash[2*l:])  # Appending the remaining portion of the hash (not utilized in this context)

        salts.append(salt)              
        pwhashs.append(pwhash)
        # Check if the current iteration is the last one in pwlist_obj
        if i == len(pwlist_obj) - 1:
        # Set up generators for brute-forcing p1 and p2
            pw_gen_1 = crypto.candidate_bf_generator(char_list, length // 2)
            pw_gen_2 = crypto.candidate_bf_generator(char_list, length // 2)
    
            # Conduct brute force attacks to identify p1 and p2 respectively
            ...
            # Combine the found p1 and p2 to reconstruct the original passwords
            results = list(map(operator.add, p1, p2))
    
    # Only during the last iteration, validate the reconstructed passwords against their hashes.
    # This validation is not essential for cracking the passwords but serves as a correctness check.
            # Iterate over pwlist_obj and validate each password and salt against their hash
            for j, (password, salt, pwhash) in enumerate(zip(results, salts, pwhashs)):
            # Assert that hashing the found password with its salt equals the stored hash
                assert pwhash_fn(salt, password) == pwhash
            # Assert that the reconstructed password and salt, when hashed, match the original hash
    
            pc.inc()  # Increment the counter or perform the relevant action after the last iteration
